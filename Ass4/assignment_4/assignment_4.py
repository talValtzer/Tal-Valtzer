...
def insert_user():
    ...
    try:
        ...
        session['msg'] = username + " added successfully"
    except Error as er:
        session['msg'] = username + " already exist"
...
def update_user_func():
    ...
    try:
        ...
        check = "select * FROM users WHERE user_name='%s';" % user_name
        ...
        if len(users_list) > 0:
            ...
            connection.commit()
            session['msg'] = user_name + " details updates"
        else:
            user_name + " do not exist"
    except Error as er:
        session['msg'] = "Error happend " + user_name
